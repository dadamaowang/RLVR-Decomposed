...
def math_equal(prediction: Union[bool, float, str],
    ...
    if timeout:
        if call_with_timeout_original(symbolic_equal_process, prediction, reference):
            return True
    else:
...
def call_with_timeout_original(func, *args, timeout=1, **kwargs):  # Changed *kwargs to **kwargs
    output_queue = multiprocessing.Queue()
