...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, advantage='positive', positive_advantage_weight=1):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        advantages, returns, advantages_before_whiten = core_algos.compute_gae_advantage_return(token_level_rewards=token_level_rewards,
                                                                      values=values,
        ...
        data.batch['advantages_before_whiten'] = advantages_before_whiten
...
def generate_consistent_uuids(length, namespace=uuid.NAMESPACE_DNS):
    return [str(uuid.uuid5(namespace, str(i))) for i in range(length)]
        ...
        for epoch in tqdm(range(self.config.trainer.total_epochs)):
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    if self.use_reference_policy and self.config.algorithm.kl_ctrl.kl_coef != 0:
                ...
                metrics["epoch"] = self.global_steps / (self.total_training_steps // self.config.trainer.total_epochs)
                metrics["seen_examples"] = self.global_steps * self.config.data.train_batch_size
                metrics["fully_solved_question_cnt"] = fully_solved_question_cnt
                metrics["fully_unsolved_question_cnt"] = fully_unsolved_question_cnt
                metrics["at_least_solve_one_time_question_cnt"] = at_least_solve_one_time_question_cnt
                fully_solved_question_cnt = 0
                fully_unsolved_question_cnt = 0
                at_least_solve_one_time_question_cnt = 0
                # collect metrics
